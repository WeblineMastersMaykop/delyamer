...
(function($) {
    ...
    Plugin.prototype.init = function() {
        ...
        self.$el.find('> li').each(function(idx) {
            ...
            // Append arrows
            sub.find('> .lz-sub').each(function() {
                $(this).prepend(self._arrowBtn());
                $(this).addClass('collapsed');
            });
            sub.find('> ul > li > .lz-sub').prepend(self._arrowBtn());

            // Add indexes and collapse all
            sub.attr('data-lz-index', idx);
            sub.find('> ul > li').each(function(subidx) {
                $(this).find('> .lz-sub').parent().attr('data-lz-index', idx + '-' + subidx);
    ...
    Plugin.prototype._addClickEvents = function(sub, idx) {
        var self = this;
        sub.find('> .lz-sub > a.arrow-btn').on('click', function(event) {
            event.stopPropagation();
            event.preventDefault();
            self._toggle(idx, 'toggle');
        });
        sub.find('> .lz-sub > span').on('click', function(event) {
            event.stopPropagation();
            event.preventDefault();
            self._toggle(idx, 'toggle');
        });
    ...
    Plugin.prototype._toggle = function(index, action) {
        var self = this;
        self.$el.find('li[data-lz-index=\'' + index + '\'] > .lz-sub > a.arrow-btn').each(function() {
            if (action === 'toggle') {
    ...
    Plugin.prototype.destroy = function() {
        $.removeData(this.$el);
        this.$el.find("> ul > li > .lz-sub > a").unbind('click');
        this.$el.find("> ul > li > .lz-sub > span").unbind('click');
    };
